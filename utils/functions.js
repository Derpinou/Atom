module.exports = {
    ...
    createGuild: function (client, guild) {
        client.databases[1].set(guild.id, {
            ...
            welcome_mp_plugin: {
                status: "disabled",
                message: "unknow"
            }, // Welcome messages in DM plugin
            leave_plugin: {
                status: "disabled",
                message: "unknow",
                channel: "unknow"
            }, // Goodbye messages plugin
            autorole_plugin: {
                status: "disabled",
                role: "unknow"
            }, // Autorole plugin
    ...
    getHex: function(color, client){

        var colours = {
            "default": client.config.embed.color,
            "aliceblue": "#f0f8ff",
            "antiquewhite": "#faebd7",
            "aqua": "#00ffff",
            "aquamarine": "#7fffd4",
    ...
    setGame: function (client){
        var ms = require('ms');
        client.user.setPresence({ activity: { name: client.config.prefix+'help on '+client.guilds.cache.size+' guilds' }, status: 'online' });
        setTimeout(function(){
            client.user.setPresence({ activity: { name: 'add me with '+client.config.prefix+' invite' }, status: 'online' });
        }, ms('15s'));
    },
    ...
    sortByKey: function(array, key) {
        return array.cache.sort(function(a, b) {
