...
class Autorole extends Command {
    ...
    constructor (client) {
        super(client, {
            ...
            dirname: __dirname,
            usage: (language) => language.get('USAGE_AUTOROLE'),
            enabled: true,
            guildOnly: true,
            aliases: [],
            ...
            nsfw: false,
            examples: (language) => language.get('EXAMPLE_AUTOROLE'),
            owner: false
        });
    }
    ...
    async run (message, args, membersdata, guild_data, data) {
        ...
        if(statut === "on"){
            ...
            // Updates db
            this.client.databases[1].set(message.guild.id+'.autorole_plugin.status', 'on');
            this.client.databases[1].set(message.guild.id+'.autorole_plugin.role', role.id);
